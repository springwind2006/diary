...
this.encodeBase64 = function(input,forUrl) {
	var output = "";
	...
	if(typeof(forUrl)!="undefined" && forUrl){
		return output.replace('=', '_').replace('/', '-').replace('+', '$');
	}
	return output;
};
...
this.encrypt = function(str,key){
	...
	for(var i=0;i<str.length;i++){
		res+=String.fromCharCode(str.charCodeAt(i) ^ key.charCodeAt(i%32));
	}
	return this.encodeBase64(res,true);
};
this.decrypt = function(result,key){
	var res="";
	key=this.md5(key);
	result=this.decodeBase64(result);
	for(var i=0;i<result.length;i++){
		res+=String.fromCharCode(result.charCodeAt(i) ^ key.charCodeAt(i%32));
	}
	return res;
};
});
